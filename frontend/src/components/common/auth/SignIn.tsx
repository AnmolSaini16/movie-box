...
export const SignIn: React.FC<Props> = ({ handleClose, setAuthAction }) => {
  ...
  useEffect(() => {
    startValidationOnType &&
      validateFields<SignInForm>(formValues, setErrorsList);
  }, [formValues]);

  const handleSignIn = async () => {
    if (!validateFields<SignInForm>(formValues, setErrorsList)) {
      setStartValidationOnType(true);
      return;
    }
    try {
      setLoading(true);
      const response = await signIn("sign-in", {
        ...formValues,
        redirect: false,
      });
      if (!response?.ok) {
      ...
      <Stack spacing={3}>
        ...
        <Box display="flex" alignItems="center" justifyContent="center">
          ...
          <Button
            variant="text"
            sx={{ fontSize: 14 }}
            onClick={() => setAuthAction(appConstants.Auth_State.SignUp)}
          >
            Create one
          </Button>
        </Box>
